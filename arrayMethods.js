...
const born16Cent = inventors.filter((b16c) => {
  return (b16c.year > 1501 && b16c.year < 1600);
});

console.log(born16Cent);
...
const birthYears = inventors.map((birthdYear) => {
  return birthdYear.year;
});
...
const fullNameInventors = inventors.map((fullNameInventor) => {
  return `${fullNameInventor.first} ${fullNameInventor.last}`;
});
...
const edison = inventors.find(lastName => lastName.last === "Edison");
console.log(edison);

//This doesnt account for other "Edison's" in the list.